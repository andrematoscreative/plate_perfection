...
const Navbar = ({ active }) => {
  ...
  const { cart } = useContext(MyContext); // Use cart from MyContext
        ...
        <ul className='app__navbar-links'>
          ...
          <li className={`cart__desktop ${active === 'Cart' ? 'active' : ''}`}>
            <Link to='/Cart'>
              ...
              <p className="app__navbar-cartcounter">{cart.length}</p> {/* Cart counter updates here */}
            </Link>
          </li>
        </ul>
              ...
              <ul>
                ...
                <li className={`cart__mobile ${active === 'Cart' ? 'active' : ''}`}>
                  <Link className='cart-mobile-link' to='/Cart' onClick={() => setToggle(false)}>
                    Cart
                    <HiOutlineShoppingCart />
                    <p className="app__hmenu-cartcounter">{cart.length}</p> {/* Cart counter updates here */}
                  </Link>
                </li>
                <div className='line2' />
              </ul>
